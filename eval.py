...
num_batch_test = np.ceil(num_data_test / batch_size).astype(np.int32)
...
with torch.no_grad():
    ...
    for batch, data in enumerate(loader_test, 1):
        ...
        print("TEST: BATCH {:04d} / {:04d} | LOSS {:.4f}".format(batch, num_batch_test,
                                                                 np.mean(loss_arr)))
        ...
        for j in range(label.shape[0]):
            ...
            plt.imsave(os.path.join(result_dir, 'png', 'label_{:04d}.png'.format(id)), label[j].squeeze(), cmap = 'gray')
            plt.imsave(os.path.join(result_dir, 'png', 'input_{:04d}.png'.format(id)), input[j].squeeze(), cmap = 'gray')
            plt.imsave(os.path.join(result_dir, 'png', 'output_{:04d}.png'.format(id)), output[j].squeeze(), cmap = 'gray')

            np.save(os.path.join(result_dir, 'numpy', 'label_{:04d}.npy'.format(id)), label[j].squeeze())
            np.save(os.path.join(result_dir, 'numpy', 'input_{:04d}.npy'.format(id)), input[j].squeeze())
            np.save(os.path.join(result_dir, 'numpy', 'output_{:04d}.npy'.format(id)), output[j].squeeze())

print("AVERAGE TEST: BATCH {:04d} / {:04d} | LOSS {:.4f}".format(batch, num_batch_test,
                                                         np.mean(loss_arr)))
